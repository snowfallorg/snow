...
pub async fn rebuild() -> Result<()> {
    println!(
        "{}",
        "Rebuilding".if_supports_color(Stdout, |t| t.bright_green())
    );
    ...
    match status {
        Ok(s) if s.success() => {
            println!(
                "{}",
                "Successfully rebuilt system".if_supports_color(Stdout, |t| t.bright_green())
            );
            // Update cache;
            println!(
                "{}",
        ...
        _ => Err(anyhow!("Failed to rebuild system")),
    }
}
